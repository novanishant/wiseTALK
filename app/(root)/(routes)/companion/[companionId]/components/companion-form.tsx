...
export const CompanionForm = ({ categories, initialData }: CompanionFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (initialData) {
        // UPDATE companinon functionality
        await axios.patch(`/api/companion/${initialData.id}`, values);
      }
      else {
        // CREATE companinon functionality
      ...
      router.refresh(); //refresh all server based components
      router.push("/");
    } catch (error) {
      toast({
        variant: "destructive",
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="col-span-2 md:col-span-1">
                  ...
                  <FormControl>
                    <Input disabled={isLoading} placeholder="CEO & Founder of Tesla" {...field} />
                  </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select
                    disabled={isLoading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}
                  >
                    <FormControl>
                      <SelectTrigger className="bg-background">
                        <SelectValue defaultValue={field.value} placeholder="Select a cagtegory" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
          ...
          <div className="space-y-2 w-full" >
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Detail instructions of AI Behaviour
              </p>
            </div>
            <Separator className="bg-primary/10" />
          </div>
